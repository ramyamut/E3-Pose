...
def training(training_im_dir,
    ...
    train_dataset = loaders.loader_rot_canonical(subj_dict=training_subj_dict,
                                        anno_df=anno_df[anno_df['fold']=='training'],
    ...
    val_dataset = loaders.loader_rot_canonical(subj_dict=val_subj_dict,
                                      anno_df=anno_df[anno_df['fold']=='validation'],
                                      augm_params=augment_params,
                                      eval=True,
                                      seed=1919)
    ...
    for epoch in range(last_epoch, n_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            with torch.set_grad_enabled(True):
                ...
                target = rot[:,:3,:3].unsqueeze(-1) #[B, 3, 3, 1]
                ...
                # compute rotation error
                    ...
                    rot_pred = torch.linalg.inv(utils.axes_to_rotation(pred[:,0], pred[:,1], pred[:,2]))
            ...
            print('iteration:{}/{}  loss:{:.5f}   rot error:{:.5f}'.format(i + 1, len(train_dataset), train_loss.item(), err_R), flush=True)
        ...
        if epoch % validate_every_n_epoch != 0:
            print('Epoch:{}  Train Loss:{:.5f}   Train rot. error:{:.5f}'.format(epoch, epoch_train_loss, epoch_train_err_R) + '\n', flush=True)
            ...
            print('Epoch:{}  Train Loss:{:.5f}  rot error:{:.3f} '.format(
                epoch, epoch_train_loss, epoch_train_err_R) + '\n', flush=True)
            print('Epoch:{}  Val Loss:{:.5f}  rot error:{:.3f} '.format(
